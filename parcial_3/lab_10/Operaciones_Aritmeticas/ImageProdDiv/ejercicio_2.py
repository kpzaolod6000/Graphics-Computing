import cv2
import numpy as np
from matplotlib import pyplot as plt
import math

def getThresholding(img,threshold):
    modifiedImg = img.copy()
    # print(img)
    for i in range(len(img)):
        for j in range(len(img[i])):
            if(img[i][j] > 135 and img[i][j] < 222): 
                modifiedImg[i][j] = np.uint8(0)
            else:
                modifiedImg[i][j] = np.uint8(255)
    return modifiedImg

imgReal1 = cv2.imread("text1.jpg", cv2.IMREAD_GRAYSCALE) #8 bit por escala de grises
imgReal1 = cv2.resize(imgReal1,(400,400))
img1 = imgReal1

imgReal2 = cv2.imread("text2.jpg", cv2.IMREAD_GRAYSCALE) #8 bit por escala de grises
imgReal2 = cv2.resize(imgReal2,(400,400))
img2 = imgReal2

cv2.imshow("Image Original 1", imgReal1)
cv2.imshow("Image Original 2", imgReal2)

img1 = img1.astype(int)
img2 = img2.astype(int)
imgResult = img1.copy()

for i in range(len(img1)):
    for j in range(len(img1[0])):
        result = img1[i,j]/img2[i,j]
        scala = (result-0)*(255-0)//(1-0) + 0
        imgResult[i,j] = scala
imgResult = imgResult.astype(np.uint8)
cv2.imshow("Resultado Division text", imgResult)

# filename = "Resultado_Division_text.jpg"
# cv2.imwrite(filename, imgResult)


f, (ax1) = plt.subplots(1, 1,figsize=(6, 6))
ax1.hist(imgResult.ravel(),256,[0,256])
ax1.set_title("Histograma")
ax1.set_xlabel('Intensidad')
ax1.set_ylabel('Cantidad de pixeles')

threshold = 120
imgThres = getThresholding(imgResult,threshold)
cv2.imshow("Resultado Thresolding", imgThres)

# filename = "Resultado_Thresolding_text.jpg"
# cv2.imwrite(filename, imgThres)


# # Filename
# plt.savefig('HistogramaThresoldingtext.png')

plt.show()
cv2.waitKey(0)
cv2.destroyAllWindows()